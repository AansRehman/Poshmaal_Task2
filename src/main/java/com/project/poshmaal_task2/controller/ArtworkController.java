...
public class ArtworkController {
    ...
    public ResponseEntity<Artwork> findById(@PathVariable("id") Long id){
        try {
            ...
            if (artwork != null) {
                return new ResponseEntity<>(artwork, HttpStatus.FOUND);
            } else {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
        }catch (Exception e){
    ...
    public ResponseEntity<Artwork> updateArtwork(@PathVariable("id") Long id, @Valid @RequestBody Artwork artwork){
        try {
            ...
            if (artwork1 == artwork) {
                return new ResponseEntity<>(artwork1, HttpStatus.CREATED);
            } else {
    ...
    public ResponseEntity<String> deleteArtwork(@PathVariable("id") Long id){
        try {
            ...
            if (res == 1.) {
                return new ResponseEntity<>("Artwork deleted successfully", HttpStatus.NO_CONTENT);
            } else {
    ...
    public List<Artwork> artworksByArtistFunc(Long id) throws Exception {
        try {
            List<Artwork> artworks = artworkRepository.findAllArtWorks();
